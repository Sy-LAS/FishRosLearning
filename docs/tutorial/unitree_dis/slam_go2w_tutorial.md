# Unitree Go2-W SLAM 部署包
[拓展坞资源](https://support.unitree.com/home/zh/developer/module_update#heading-7)
[官方对于go2的slam接口介绍](https://support.unitree.com/home/zh/developer/SLAM%20and%20Navigation_service)
> 目标文件夹：~\slam\unitree\go2w\unitree`
>
> 这是宇树官方提供的 **Go2-W 轮足机器人 SLAM 运行时部署包**。。。
>
> 与 `go2/` 版本的关系：两包的 **目录结构与全部文本配置文件完全一致**（包括 `config/*`、`rviz2/*`、`example/*`）。差异只体现在以下几个二进制上：`bin/unitree_slam`、`lib/libcommand_function.so`、`lib/libslam_server.so`、`lib/libodom_imu_data.so` 为针对 Go2-W 轮足平台（主要差异：移动方式为轮式+腿部、底盘动力学模型与 PID 阈值不同）重新编译的版本。上层话题名与 API ID 与 `go2/` 保持一致。
...
```
G:\XJTLU\SURF\SLAM\UNITREE\GO2W\UNITREE
+---lib
+---module
|   \---unitree_slam
|       +---bin
...
- **keyDemo**：ELF 可执行文件，由同目录下 `example/src/keyDemo.cpp` 编译而来；运行 `./keyDemo eth0` 即可通过键盘（q/w/a/s/d/f/z/x）触发 **开始建图 / 结束建图 / 重定位 / 添加目标点 / 执行巡航 / 清空任务 / 暂停 / 恢复** 等 SLAM 操作。该二进制与 `go2/` 包下的 `keyDemo` 字节一致，因为示例仅调用 DDS API，与底盘无关。
- **mid360_driver**：Livox MID-360 激光雷达的用户态驱动，负责按 `config/mid360/*` 中的网络与扩展参数拉起雷达、发布点云与 IMU 到 DDS 话题 `rt/unitree/slam_lidar/points`、`rt/unitree/slam_lidar/imu`。Go2-W 默认使用 MID-360。
- **unitree_slam**：核心 SLAM 服务进程（10 MB 级），加载 `lib/` 下的功能库，订阅雷达与机器人里程计、IMU，承担**前端匹配、建图、重定位、路径规划与 PID 位姿控制**，并通过 DDS 服务接口 `slam_operate` 暴露建图/重定位/导航 API。**此二进制相对 `go2/` 版本经过轮足平台重编译**，内部会选择 `config/*.yaml` 中的 `Go2_W` 分段参数。
- **xt16_driver**：Hesai PandarXT-16 机械旋转激光雷达的用户态驱动，按 `config/xt16/*` 中的 IP、网卡与标定信息拉起雷达并发布点云；用于 XT-16 雷达的兼容模式。
...
- **config.yaml**：实时 2D 栅格地图（碰撞检测 / 预警）参数，按机型分区（B2、B2_W、G1、Go2、Go2_W）定义分辨率、宽度、雷达安装外参 `T_Dog2lidar`、雷达高度、订阅的点云/里程计话题、碰撞盒与预警盒尺寸、预测碰撞的步数与时间，以及日志路径 `logs/gridmap/`。本包实际生效段为 `Go2_W`：`revolution=0.05`、`wide=6.0`、`lidar_height=0.4`、外参 `[0, 0, 0, 0, 0.226, 0]`。
...
- **pose_ctrl_param.yaml**：按机型分区（B2、B2_W、G1、Go2、Go2_W、H1）的位姿级 PID 控制参数（x/y/yaw 的 Limmax/Vmax/KP/KI/KD 与阈值）及规划细粒度阈值；本部署包的默认生效组为 `Go2_W`（相较 Go2 的 x 方向速度保持 1.8、但 yaw KP 由 8.0 降为 2.0，更适合轮足底盘的转向特性）。

### module/unitree_slam/config/pl_mapping/
- **mid360.yaml**：MID-360 激光雷达下的 **FAST-LIO 风格点云-IMU 紧耦合建图** 参数，分机型给出 IMU/Lidar 话题、协方差、地图分辨率、雷达到 IMU 的外参 `pose_imu_lidar`、保存 pcd 开关（`save_pcd`）等；本包默认读取 `Go2_W` 段，使用 `rt/unitree/slam_lidar/points` 与 `rt/unitree/slam_lidar/imu`。
- **xt16.yaml**：XT-16 激光雷达下的建图参数，`Go2_W` 段使用较低的 IMU 倍率（`imu_acc_multi=1.0`）、较大的 `filter_num=35` 与外参四元数 `[0.707, 0, 0, 0.707, 0.171, 0, 0.0908]`。

### module/unitree_slam/config/pl_relocation/
- **mid360.yaml**：MID-360 下的 **点云重定位**（global-to-local ICP）参数，`Go2_W` 段相比 `Go2` 段把 `icp_score` 从 0.03 放宽到 0.05，其他字段一致；包含发布话题（`rt/unitree/slam_relocation/odom`、`.../global_map`、`.../local_map`）、ICP 分数阈值、近邻网格尺寸、重定位容差、速度上限 `speed_max=2.5 m/s` 等。
...
- **param.yaml**：按机型（B2、B2_W、Go2、Go2_W、G1、H1）给出 SLAM 服务的运行时选项：网卡、激光雷达类型（`mid360` / `xt16`）、雷达序列号与 IP、订阅的狗身 odom 与 IMU 话题 `rt/dog_odom` / `rt/dog_imu_raw`、连接超时秒数。Go2_W 段默认 `lidar_type=mid360`、`lidar_ip=192.168.123.20`，是本包默认加载项。
...
- **libbasic_data.so**：SLAM 基础数据结构与工具库（预编译），上层其他 `lib*.so` 与 `unitree_slam` 均依赖；提供日志、几何类型、时间戳等基础设施。（与 `go2/` 版本字节一致）
- **libcommand_function.so**：命令与任务调度库，承载 DDS 服务端对外 API 的分发逻辑（把收到的 `apiId + json` 转成内部函数调用），与 `keyDemo.cpp` 中注册的 1801/1802/1804/1102/1201/1202/1901 一一对应。**Go2-W 版相对 Go2 版体积更小**，因其裁掉了一些四足专属的逻辑分支，转而走轮足底盘的控制通道。
- **libgraph_function.so**：拓扑/位姿图相关功能库，负责 SLAM 后端的节点/边管理（如关键帧与回环图操作）。（与 `go2/` 版本字节一致）
- **libgrid_map_core.so**：来自 ANYbotics `grid_map` 家族的核心库，提供多层栅格地图的底层存储、插值与序列化，服务于本包的 `gridmap_config` 功能。（与 `go2/` 版本字节一致）
- **liblivox_lidar_sdk_shared.so**：Livox 官方 Lidar SDK2 的共享版本，被 `mid360_driver` 与 `unitree_slam` 调用，实现 MID-360 的网络通信、时间同步与点云解析。（与 `go2/` 版本字节一致）
- **libodom_imu_data.so**：里程计与 IMU 数据聚合库，负责把机器人身上的 `rt/dog_odom`、`rt/dog_imu_raw`、激光 IMU 做时间对齐与坐标转换，对外输出统一的状态流给 SLAM 前端。**相对 Go2 版本略有差异**，主要适配轮足底盘下腿部编码器与轮速里程计的融合逻辑。
- **libPandarGeneralSDK.so**：Hesai Pandar 通用 SDK，被 `xt16_driver` 调用以解析 XT-16 UDP 数据包并发布点云。（与 `go2/` 版本字节一致）
- **libslam_server.so**：SLAM 服务封装，把建图、重定位、导航流程包装为 Unitree 风格的 DDS 服务端点，供 `keyDemo` 等客户端通过 `Client::Call` 调用。**Go2-W 版相对 Go2 版有小幅差异**，主要体现在默认加载的机型分段（读取 `Go2_W` 而非 `Go2`）与轮足运动学适配。
- **libsport_control.so**：运动控制桥接库，负责把规划器输出的目标速度/位姿转成 `unitree_sdk2` 下 Sport/Loco 客户端可接收的指令，驱动 Go2-W 真正移动。（与 `go2/` 版本字节一致，因为上层指令接口相同，底盘差异由机器人侧的运动控制器处理）
...
## 附：Go2-W 与 Go2 运行时差异速查

| 维度 | Go2（纯四足） | Go2-W（轮足） |
| --- | --- | --- |
| `gridmap_config/config.yaml` 生效段 | `Go2`：分辨率 0.1 m、宽度 8 m、外参 `[0.1701, 0, 0.0908, 0, 0, 1.57]` (xt16) | `Go2_W`：分辨率 0.05 m、宽度 6 m、外参 `[0, 0, 0, 0, 0.226, 0]` (mid360) |
| `pose_ctrl_param.yaml` 生效段 | `Go2`：yaw KP 8.0 / KI 0.5（四足转向较灵活） | `Go2_W`：yaw KP 2.0 / KI 0.1（轮足转向更刚） |
| `pl_relocation/mid360.yaml` | `Go2`：`icp_score=0.03` | `Go2_W`：`icp_score=0.05`（容差略放宽） |
| `slam_interfaces_server_config/param.yaml` | `Go2` 段 | `Go2_W` 段 |
| 重编译的二进制 | —— | `bin/unitree_slam`、`lib/libcommand_function.so`、`lib/libslam_server.so`、`lib/libodom_imu_data.so` |
| 字节一致的二进制 | —— | `keyDemo`、`mid360_driver`、`xt16_driver` 以及其他 `.so` |
